...
matrix = pd.read_csv("https://raw.githubusercontent.com/wildliferesilience/wildliferesilience/main/docs/input/MetaNetwork_Matrix_LTB_June.csv", header=None).values

# Load the node names from a CSV file with the specified column names
names = pd.read_csv("https://raw.githubusercontent.com/wildliferesilience/wildliferesilience/main/docs/input/MetaNetwork_Matrix_Label_LTB_June.csv", usecols=["id", "guild", "G", "taxon"])
...
fig.update_layout(title='Network Graph', showlegend=False, hovermode='closest', margin=dict(b=20, l=5, r=5, t=40))
