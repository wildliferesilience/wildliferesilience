...
## Tahoe-Central Sierra Landsapce

The Tahoe-Central Sierra (TCSI) landscape is a 1 million ha section of Central California forested land. The TCSI is home to a high diversity of vegetation communities, flora, and fauna.
...
There are 14 dominant vegetation types in the TCSI that vary in their occurrence (A) and density across the gradient (B[^1]).

[^1]: The vegetation types that occur in the TCSI vary spatially (A) and in their occurrence at different elevations (B). Colors used for vegetation types in figure B correspond to vegetation type colors in figure A.

![](SVG_images/TCSI_map_density.svg){fig-align="center" width="100%"}

Considering wildlife communities at the scale of the TCSI landscape is critical because wildlife communities occur at large scales. Therefore, working at a spatial scale that encompass landscape level community processes (e.g., dispersal, connectivity, patch size, habitat diversity, neighborhood effects etc.) and disturbances (e.g., large fires, climate change, large scale management) can impact our evaluations of current communities and predictions into the future.

Further the TCSI landscape is an area of active research[^2] and presents many opportunities to collaborate and interact with an established group of stakeholders.

[^2]: (see [Resources](resources.qmd) for more information)
...
Lake Tahoe basin is located between two biogeographic provinces, the Great Basin and the Sierra Nevada, and supports high biodiversity. There are over 1,200 species of plants and animals in the basin. The basin also has a steep elevational gradient which means that it spans four life zones: lower montane, upper montane, subalpine, and alpine. Eight major vegetation types are common within the Lake Tahoe basin (A) and they range in their density across the elevational gradient (B[^3]).

[^3]: The vegetation types that occur in the Lake Tahoe basin vary spatially (A) and in their occurrence at different elevations (B). Colors used for vegetation types in figure B correspond to vegetation type colors in figure A.

![](SVG_images/LTB_map_Dist.svg){fig-align="center" width="90%"}

When comparing the plants and animals that occur in those eight vegetation types, we've found that generally, overlap in species occurrence is more common for vegetation types in the same life zone. For example, (A) Jeffrey pine (JPN, orange circle), Sierran mixed conifer (SMC, dark green circle), and White fir (WFR, purple circle) forests are commonly found in the lower montane zone (B[^4]) and they have high overlap in the types of plants and animal that occur in those forest types.

[^4]: In this figure, each large circle is a vegetation type, and each small circle is a species (green vertebrates, gold plants). Lines connecting species to vegetation types indicate that the species occurs in that vegetation type. Species may occur in multiple vegetation types (have multiple connecting lines) or occur in one vegetation type.

![](SVG_images/LTB_Overlap_Dist.svg){fig-align="center" width="100%"}

Given the different (and similar) species present in these different communities, we next use our food web models and metrics to see if communities differ in composition, structure, and function. To see what this looks like see the [applications](applications.qmd) page. 
